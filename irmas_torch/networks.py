...
class BottleneckBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, *args, **kargs):
        ...
        self.rconv1 = RConv(out_channels, out_channels, kernel_size, stride=(1, 1))
        self.rconv2 = RConv(
            out_channels, 4 * out_channels, kernel_size=(1, 1), stride=(1, 1)
        )
    ...
    def __init__(
        ...
        **kargs,
        ...
        for i in range(self.size):
            if i == 0:
                self._modules[f"block{i:03d}"] = self.cons(
                    in_channels, out_channels, kernel_size, strides, *args, **kargs
                )
            else:
                self._modules[f"block{i:03d}"] = self.cons(
                    out_channels, out_channels, kernel_size, (1, 1), *args, **kargs
                )
