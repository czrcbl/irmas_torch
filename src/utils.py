...
def get_network(base_network, pretrained, mono):
    in_channels = 1 if mono else 2
#    if args.test:
#        net = TestNet(in_channels, 11).to(device)
    if base_network == 'resnet18':
        base_net = torchvision.models.resnet18(pretrained=pretrained)
        last_channel = 512
    elif base_network == 'resnet34':
        base_net = torchvision.models.resnet34(pretrained=pretrained)
        last_channel = 512
    elif base_network == 'resnet50':
        base_net = torchvision.models.resnet50(pretrained=pretrained)
        last_channel = 2048
    elif base_network == 'resnet101':
        base_net = torchvision.models.resnet101(pretrained=pretrained)
        last_channel = 2048
